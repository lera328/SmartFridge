...
import com.example.smartfridge.shopping_list.ShoppingListCardView_adapter
...
class ShoppingListFragment : Fragment() {
    ...
    private lateinit var adapter: ShoppingListCardView_adapter
    private lateinit var modelArrayList: ArrayList<Списки_покупок>
    override fun onCreateView(
        ...
        modelArrayList = arrayListOf<Списки_покупок>()
        ///


        // Добавляем данные в массив
        modelArrayList.add(
            Списки_покупок(1,1,"Молоко",false)

        )
        // Добавьте остальные элементы списка здесь.

        adapter = ShoppingListCardView_adapter(requireContext(), modelArrayList)
